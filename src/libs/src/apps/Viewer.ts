...
export class Viewer {
  ...
  private initRenderer() {
    ...
    this.renderer.setClearColor(this.data && this.data.renderer.color ? this.data.renderer.color : 0xcccccc, 1);
  ...
  private initView() {
    ...
    if (this.sideViews.length > 0) {
      for (let i = 0; i < this.sideViews.length; i++) {
        this.sideViews[i].setCamera(this.sideViews[i].camera as THREE.Camera, this.sideViews[i].viewData.camera);
      }
    }
  }
  ...
  async start(url: any, modelData: any, data: any) {
    ...
    if (data) {
      ...
      if (this.content)
        this.content.setTransform('env', 'rotation', data.env && data.env.rotation ? data.env.rotation : [0, 0, 0]); // TODO: content
    }
    // clip
      ...
      case 'camera':
        ...
        if (type === 'angle' || type === 'zoom' || type === 'pan') {
          ...
          if (this.mainView && this.data && this.mainView.camera) {
            ...
            this.setDefaultCamera(this.mainView.camera, this.mainView.viewData.camera);
          }
    ...
    if (selected.isComponent) selected.setTransform(type, value);
      ...
      if (selected === 'shadow' && this.content) {
        ...
        Array.isArray(value)
          ...
          : (selected[type] = value);
      else Array.isArray(value) ? selected.fromArray(value) : selected.set(value.x, value.y, value.z);
    }
    this.needsUpdate = true;
  }
  ...
  resetViewLimit() {
    ...
    if (this.data && this.data.control) {
      if (this.mainView) this.mainView.setControlLimit(this.data.control.limitation);
      for (let i = 0; i < this.sideViews.length; i++) {
        const view = this.sideViews[i];
  ...
  updateAspectRatio() {
    ...
    } else if (this.sideViews.length > 0 && this.viewMode === VIEW_MODE.MULTIPLE) {
      for (let i = 0; i < this.sideViews.length; i++) {
  ...
  onDocumentMouseMove(event: any) {
    // TODO: sideViews
    this.mainView && this.mainView.picker && this.mainView.picker.onDocumentMouseMove(event);
    this.needsUpdate = true;
  }

  onDocumentMouseDown() {
    // TODO: sideViews
    this.mainView && this.mainView.picker && this.mainView.picker.onDocumentMouseDown();
  }

  onDocumentMouseUp() {
    // TODO: sideViews
    this.mainView && this.mainView.picker && this.mainView.picker.onDocumentMouseUp();
    this.needsUpdate = true;
  }
  ...
  takeScreenshot() {
    ...
    const imgData = this.renderer ? this.renderer.domElement.toDataURL(imgType) : '';
